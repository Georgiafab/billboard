...
export default function IndexMobie({ pageChange, total, data }: Props) {
  return (
    <>
      <div className='px-4'>
        {data.map(item => (<div key={item.id} className={` rounded-t-xl ${item.audstatus === AUD_STATUS.pending ? 'bg-[#2C2B50]' : item.audstatus === AUD_STATUS.success ? 'bg-green' : 'bg-orange'} pt-5 pb-8 px-5`}>
